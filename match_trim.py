...
def match2hst(lis_f,hst_tab_ref, hst_tab_red, tdist=None, hst_ref=False):
    '''
    matches the lis file from Starfinder with the HST reference
    uses the fact that the first star is in the HST Reference (by name) to trim down the Stars from HST that we try to match with
    if hst_Ref =True, then coordinates are returned in ACS coordiantes
    hst_ref = False, then coordinates are returned in the individual acs coordinates
    '''
    ...
    if tdist != None:
        dx , dy = tdist.evaluate(stf['col1'],stf['col3'])
        stfx = stf['col4'] + dx
        stfy = stf['col5'] + dy
    else:
        stfx = stf['col4']
    ...
    idx1 , idx2 , dm, dr = jay.match(stf['col4'], stf['col5'] ,stf['col2'], xnew, ynew, hst['Mag'], 6)
...
def match2hst_err(lis_f, hst,fits_file,  hst_ref=False, ecut=.002, hpix_scale=.05, ap_dar=True):
    '''
    matches the lis_f is fits table of coordinates that was generated by stack_precision
    uses the fact that the first star is in the HST Reference (by name) to trim down the Stars from HST that we try to match with
    if hst_Ref =True, then coordinates are returned in ACS coordiantes
    hst_ref = False, then coordinates are returned in the individual acs coordinates
    ecut is the eventual error cut for caluclating te 4 parameter tranformation (in arcseconds)
    ...
    if ap_dar:
        xhst, yhst = applyDAR_coo(fits_file,hst['Xarc'], hst['Yarc'], orient_ang=-102.5, hst_pix_scale=hpix_scale)
    else:
        xhst = hst['Xarc']
        yhst = hst['Yarc']
    ...
    return   stf['col1'][idx1], stf['col3'][idx1], xnew[idx2], ynew[idx2], hst['Name'][idx2], stf['col5'][idx1], hst['Mag'][idx2], (hst['xerr'][idx2]*hpix_scale)/stf_pix_scale, (hst['yerr'][idx2]*hpix_scale)/stf_pix_scale, stf['col2'][idx1], stf['col4'][idx1]

def match2hst_err2(lis_f, ref,fits_file, ecut=.002, ref_scale=1.0, ap_dar=True):
    '''
    ...
    idhst = match_by_name(stf['col0'], ref['col0'])

    if ap_dar:
        xhst, yhst = applyDAR_coo(fits_file,ref['col1'],ref['col3'], orient_ang=0, hst_pix_scale=ref_scale)
    else:
        xhst = ref['col1']
        yhst = ref['col3']
        
    #do error cut, then recalulcate 4 parameter tranforamtion of HST
    hst_ebool = (ref['col2'][idhst] < ecut / ref_scale )*(ref['col4'][idhst] < ecut / ref_scale)
    stf_ebool = (stf['col2'] < ecut / stf_pix_scale)*(stf['col4'] < ecut / stf_pix_scale)
    tbool = hst_ebool * stf_ebool
    

    cx,cy = high_order.four_param(xhst[idhst][tbool], yhst[idhst][tbool], stf['col1'][tbool], stf['col3'][tbool])
    xnew = cx[0] + cx[1]*ref['col1'] + cx[2]*ref['col3']
    ynew = cy[0] + cy[1]*ref['col1'] + cy[2]*ref['col3']

    idx1 , idx2 , dm, dr = jay.match(stf['col1'], stf['col3'] ,stf['col5'], xnew, ynew, ref['col5'], 8)                            
        
            
    print 'found ',len(idx1),' matches out of  ',len(stf['col4'])
    
    return   stf['col1'][idx1], stf['col3'][idx1], xnew[idx2], ynew[idx2], ref['col0'][idx2], stf['col5'][idx1], ref['col5'][idx2], (ref['col2'][idx2]*ref_scale)/stf_pix_scale, (ref['col4'][idx2]*ref_scale)/stf_pix_scale, stf['col2'][idx1], stf['col4'][idx1]
...
def precision_stack(lis_files, hst_tab_ref, hst_tab_red, n_iter=3, order=3, plot_hist=False, pix_range=10, long_in_set=None, tab_format='fits'):
    '''
    ...
    for n in range(n_iter):
        ...
        for i, lis_i in enumerate(lis_files):
            x,y,xr,yr,name, hst_bool, m_idx, mag,hst_mag,  hst_xerr, hst_yerr = match2hst(lis_i, hst_tab_ref, hst_tab_red, tdist=t)
    ...
    lis_bool= np.ones(len(lis_files), dtype='bool')
    ...
    while np.sum(lis_bool) > 0:
                ...
                if ii+fnum < len(xstf):
                    ...
                    if N > 9:
                        ...
                        if np.mean(x2m-x1m)**2+np.mean(y2m-y1m)**2 < pix_range**2:
                            ...
                            idx1 , idx2 , dm, dr = jay.match(xstf[fnum], ystf[fnum], mstf[fnum], xnew, ynew, mstf[ii+fnum], 8)
        ...
        #now calc precisions for all lists and write a series of files !
            ...
            if x != []:
                ...
                tab = Table(data=[name_f, x, xerr, y, yerr,xref1, xrerr, yref1, yrerr, mag, N])
        ...
        out_fits = []
        for s in lis_files[fits_frames_index]:
            out_fits.append('../../Data/'+str(s).replace('_0.8_stf.lis', '.fits'))
...
def distortion_solution_err(lis_files, hst_tab_ref ,DAR_fits, n_iter=1, order=3, plot_hist=False, align=1, ap_dar=True):
    '''
    go through all the starfinder , match into hst, find the deltas in the keck frame
    then solve for s distortion soltion
    NOTE THAT REFERENCE ERRORS ARE ALL RETURNED IN NIRC2 PIXEL SCALE 
    '''
    ...
    for n in range(n_iter):
        ...
        for index, i in enumerate(lis_files):
            if align == 1:
                x,y,xr,yr, name, s_mag, fmag,hst_xerr, hst_yerr, stf_xerr, stf_yerr = match2hst_err(i, hst_tab_ref, DAR_fits[index], ap_dar=ap_dar)
            elif align ==2:
                x,y,xr,yr, name, s_mag, fmag,hst_xerr, hst_yerr, stf_xerr, stf_yerr = match2hst_err2(i, hst_tab_ref, DAR_fits[index], ap_dar=ap_dar)
        ...
        weights = 1  / (np.array(xerr)**2+np.array(yerr)**2+np.array(hxerr)**2+np.array(hyerr)**2)**0.5        
        t,dx,dy = calc_sig_dist(np.array(xstf), np.array(ystf), np.array(xref)-np.array(xstf),np.array(yref)-np.array(ystf), order=order, n_iter=15, weights=weights)
        if plot_hist:
            plt.figure(1)
            plt.clf()
            ...
            plt.figure(2)
            plt.clf()
            plt.title('Y residaul')
            plt.hist(dy,bins=100, range=(-5,5))
            plt.show()
            
        
        print 'total of ', len(xstf),'  matches found'


    #fit for distortion
    t = high_order.LegTransform(np.array(xstf), np.array(ystf), np.array(xref)-np.array(xstf),np.array(yref)-np.array(ystf), 3)
...
def match_and_write(outfile='april_pos.txt', fits_lis='first_fits_m.lis'):
    ...
    x,xerr,y,yerr,xref,xreferr,yref,yreferr,t,hmag, name,frame_num, frame_name = distortion_solution_err(lis_fits['col2'], hst_ref, lis_fits['col1'], n_iter=1)
...
def match_and_write2(reffile='NIRC2_leg_reference.txt',fits_lis='first_fits_m.lis', outfile='april_pos_leg_r2.txt', ap_dar=True):
    ...
    x,xerr,y,yerr,xref,xreferr,yref,yreferr,t,hmag, name, frame_num, frame_name = distortion_solution_err(lis_fits['col2'], hst_ref,lis_fits['col1'], n_iter=1, align=2, ap_dar=ap_dar)
    outtab = Table(data=[name, hmag, x, xerr, y, yerr, xref , xreferr, yref , yreferr, frame_num, frame_name], names=['name','mag', 'x', 'xerr', 'y', 'yerr', 'xr', 'xrerr', 'yr', 'yrerr', 'frame_num', 'frame_name'])
...
def applyDAR_coo(fits, x_h, y_h, orient_ang=-102.5, hst_pix_scale=.05):
    """
    ...
    xhcen = np.median(x_h)
    yhcen = np.median(y_h)
    x = ((x_h - xhcen) * math.cos(math.radians(orient_ang)) + (y_h - yhcen) * math.sin(math.radians(orient_ang))) / hst_pix_scale
    #need to be reversed, so that x + to the west ?????
    #x = -1.0 * x
    y = (-1.0*(x_h - xhcen) * math.sin(math.radians(orient_ang)) + (y_h - yhcen) * math.cos(math.radians(orient_ang)))  / hst_pix_scale
    ...
    #x needs to be reversed again, back to original
    xnew = xnew #* -1.0
    #now rerotate
    xout1 = (xnew * math.cos(math.radians(orient_ang)) - ynew * math.sin(math.radians(orient_ang))) * hst_pix_scale
    yout1 = (xnew * math.sin(math.radians(orient_ang)) + ynew *math.cos(math.radians(orient_ang))) * hst_pix_scale
    return xout1 + xhcen, yout1 + yhcen 
...
def dist_sol_dum(pos_txt, sigma=25):
    '''
    dumdundundundundundundundun

    want to get gaussian weighted smothed
    soltuion:
    throw gaussian for all data poitns within 4*sigma of point in lookup table you wish to genreate
    take weidghted mean to get distortino
    weights are the amplitude of the gaussians at the point you evaluate, and the means are the x/y distortin measurments for that point.

    DOITRIGHTNOWWHYHAVENTYOUWRITTENTHISYET
    '''
    pass
